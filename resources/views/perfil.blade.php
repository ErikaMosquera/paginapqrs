...
<div class="card w-1/2 shadow-2xl bg-base-100 mt-6">
    <div class="card-body">
        ...
        {{-- Utiliza el método 'PUT' en el formulario --}}
        <form action="{{ route('perfil.update', ['user' => $user->id]) }}" method="POST">
            @csrf
            {{-- Usamos el método 'PUT' a través del campo '_method' --}}
            @method('PUT')
...
<input type="password" name="password_confirmation" placeholder="Confirme la
nueva contraseña" class="input input-bordered" maxlength="45" required />
</div>
<div class="form-control mt-6 w-1/2">
